...
const SpicyLister = () => {
  ...
  const [userEmail, setUserEmail] = useState(''); // New: for supporter tracking
  ...
  // NEW: Enhanced supporter tracking with email collection
  const handlePayPalUpgrade = async () => {
    ...
    // Set premium after payment (simplified for now)
    setTimeout(() => {
  ...
  // Enhanced Premium Modal Component with supporter tracking
  const PremiumModal = () => (
    showPremiumModal && (
