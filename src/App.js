...
const SpicyLister = () => {
  ...
  const handleCoffeeConfirm = (confirmed) => {
    ...
    if (confirmed) {
      ...
      setIsPro(true);
      setProExpiryDate(d);
      alert(`🎉 PRO for 1 month! Valid until ${d.toLocaleDateString()}`);
  ...
  <div style={{ fontFamily: 'sans-serif', background: "#fffaf3", minHeight: "100vh", paddingBottom: 80 }}>
        ...
        {images.map(img=>(
          <div key={img.id} style={{position:"relative"}}>
            ...
            <button title="Remove" onClick={()=>removeMedia(img.id,'img')}
              style={{position:"absolute",top:-7,right:-7,background:"#fa5027",color:"#fff",border:"none",borderRadius:"50%",width:22,height:22,cursor:"pointer"}}>✖</button>
          </div>
        ))}
    ...
    {result &&
      <div style={{
        ...
        <h2 style={{color:"#b4561f",marginTop:0}}>🎉 AI Listing Generated:</h2>
    ...
    {showProModal && (
      <div style={{
        ...
        <div style={{
          ...
          >☕ Buy Me a Coffee (1mo Pro)</button>
          <br/>
